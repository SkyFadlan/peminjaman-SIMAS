@php
    ...
    $statusClass = $pinjam->status == 'disetujui' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800';
    $statusLabel = $pinjam->status == 'disetujui' ? 'Disetujui' : 'Dipinjam';
    
    $sekarang = now();
    
    if ($pinjam->tipe_pinjam == 'hari') {
        $tglKembali = \Carbon\Carbon::parse($pinjam->tanggal_kembali)->startOfDay();
        $tglKembaliText = $tglKembali->format('d M Y');
        $selisih = $sekarang->startOfDay()->diffInDays($tglKembali, false);
        $labelSisa = abs($selisih) . " hari";
    } else {
        $jamKembali = \Carbon\Carbon::parse($pinjam->jam_kembali);
        $tglKembaliText = $jamKembali->format('d M Y H:i');
        $selisih = $sekarang->diffInHours($jamKembali, false);
        $labelSisa = abs($selisih) . " jam";
    }
...
<tr class="hover:bg-indigo-50 transition-colors">
    ...
    <td class="px-6 py-4 text-sm text-gray-900">{{ $tglKembaliText }}</td>
    <td class="px-6 py-4">
        <span class="px-3 py-1 {{ $statusClass }} rounded-full text-xs font-medium">{{ $statusLabel }}</span>
    </td>
    <td class="px-6 py-4">
        @if($selisih > 0)
            <span class="text-xs font-semibold text-blue-600">{{ $labelSisa }} lagi</span>
        @elseif($selisih == 0)
            <span class="text-xs font-semibold text-orange-600">Hari ini</span>
        @else
            <span class="text-xs font-semibold text-red-600">Terlambat {{ $labelSisa }}</span>
        @endif
    </td>
    <td class="px-6 py-4">
